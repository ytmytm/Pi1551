    ...
    switch (secondaryByte & 0xF0)
    {
        ...
        case SEC_SECOND:
        {
            ...
            else
            {
                ...
                if (ch.command[0] == '$')
                {
                    PrepareDirectoryResponse(channel);
                }
                else
                {
                    PrepareLoadChannel(channel);
...
bool TCBM_Commands::PrepareLoadChannel(u8 channel)
{
    secondaryAddress = channel;
    Channel& ch = channels[channel];
...
void TCBM_Commands::PrepareDirectoryResponse(u8 channel)
{
    Channel& ch = channels[channel];
    ch.cursor = 0;
    ch.bytesSent = 0;
...
void TCBM_Commands::PrepareStatusResponse()
{
